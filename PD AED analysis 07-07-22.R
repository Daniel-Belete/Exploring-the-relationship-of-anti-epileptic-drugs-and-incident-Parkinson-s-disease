...
#NEED TO EXCLUDE SPECIAL FIGURES (1 AND 3)
...
df5$eid = factor(df5$eid)

df5 = left_join(df5,pdmeds, by=c("eid"="eid"))
df5$L_dopa_status[is.na(df5$L_dopa_status)] = 0
df5$L_dopa_status = factor(df5$L_dopa_status)

ICD_AED=glm(data=df5,
            L_dopa_status~`Sex.0.0`+`Year.of.birth.0.0`+`Townsend.deprivation.index.at.recruitment.0.0`+
              factor(df5$AED_ICD_status),
            family=binomial(link="logit"))
summary(ICD_AED)

ICD_KEP=glm(data=df5,
            L_dopa_status~`Sex.0.0`+`Year.of.birth.0.0`+`Townsend.deprivation.index.at.recruitment.0.0`+
              factor(df5$KEP_ICD_drug_status),
            family=binomial(link="logit"))
summary(ICD_KEP)

ICD_LAM=glm(data=df5,
            L_dopa_status~`Sex.0.0`+`Year.of.birth.0.0`+`Townsend.deprivation.index.at.recruitment.0.0`+
              factor(df5$LAM_ICD_drug_status),
            family=binomial(link="logit"))
summary(ICD_LAM)

ICD_SV=glm(data=df5,
           L_dopa_status~`Sex.0.0`+`Year.of.birth.0.0`+`Townsend.deprivation.index.at.recruitment.0.0`+
             factor(df5$SV_ICD_drug_status),
           family=binomial(link="logit"))
summary(ICD_SV)

ICD_CB=glm(data=df5,
           L_dopa_status~`Sex.0.0`+`Year.of.birth.0.0`+`Townsend.deprivation.index.at.recruitment.0.0`+
             factor(df5$CB_ICD_drug_status),
           family=binomial(link="logit"))
summary(ICD_CB)
...
write.csv(df2, "/data/Wolfson-UKBB-Dobson/drugs/newcasecontrolPDmedsensOR.csv")
...
write.csv(PD_table, "/data/Wolfson-UKBB-Dobson/drugs/newcasecontrolSROR.csv")

#make case control group (epilepsy sensitivity)
...
selected_vars = read_csv("/data/Wolfson-UKBB-Dobson/drugs/epilepsy_dx_06_21.csv")
selected_vars = selected_vars %>% select(EID,Epilepsy_status_06_21)
selected_vars$EID = factor(selected_vars$EID)
gp_df$eid = factor(gp_df$eid)
gp_df = left_join(gp_df,selected_vars,by=c("eid"="EID"))
gp_df = gp_df %>% filter_at(vars(contains("Epilepsy_status_06_21")), any_vars(. != 1))
...
write.csv(gp_df2,"/data/Wolfson-UKBB-Dobson/drugs/newcasecontrolgroupepil.csv")
...
df = read_csv("/data/Wolfson-UKBB-Dobson/drugs/newcasecontrolgroupepil.csv")
...
write.csv(df5,"/data/Wolfson-UKBB-Dobson/drugs/newcasecontrolgroupepil2.csv")
...
df = read_csv("/data/Wolfson-UKBB-Dobson/drugs/newcasecontrolgroupepil2.csv")
df$eid = as.character(df$eid)
...
write.csv(merged_df,"/data/Wolfson-UKBB-Dobson/drugs/newICDcasecontrolsetepil.csv")
...
df = read_csv("/data/Wolfson-UKBB-Dobson/drugs/newICDcasecontrolsetepil.csv")
...
write.csv(df2, "/data/Wolfson-UKBB-Dobson/drugs/newcasecontrolICDORepil.csv")
